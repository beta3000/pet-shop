  ...
  return (
    <div>
      <h2 id="pet-heading" data-cy="PetHeading">
        <Translate contentKey="petShopApp.pet.home.title">Pets</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="petShopApp.pet.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="petShopApp.pet.home.createLabel">Create new Pet</Translate>
          </Link>
        </div>
      </h2>
        ...
        {petList && petList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="petShopApp.pet.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {petList.map((pet, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{pet.name}</td>
                  <td>{pet.petType ? <Link to={`pet-type/${pet.petType.id}`}>{pet.petType.name}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="petShopApp.pet.home.notFound">No Pets found</Translate>
            </div>
          )
        )}
      </div>
